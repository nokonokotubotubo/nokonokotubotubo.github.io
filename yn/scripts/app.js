// YourNewsApp - NGドメイン機能削除・フィードバック重複防止完全対応版
class YourNewsApp {
    constructor() {
    ...
    async processFeedback(articleId, feedback) {
        try {
            ...
            // 【修正】NGドメインチェックを削除、興味あり/なしのみ受付
            if (feedback !== 1 && feedback !== -1) {
                console.error('Invalid feedback type:', feedback);
                return false;
            }
            ...
            article.feedbackHistory.push({
                type: feedback === 1 ? 'interest' : 'disinterest',
    ...
    async recalculateAllArticlesInterest() {
        try {
            ...
            if (updatedCount > 0) {
                ...
                if (ngArticleCount > 0) {
                    message += `（${ngArticleCount}件をNGワード記事として処理）`;
                }
            ...
            console.log(`✅ 全記事再計算完了: ${updatedCount}件更新, ${ngArticleCount}件NGワード処理`);
    ...
    debugFeedbackState() {
        ...
        const stored = localStorage.getItem(this.FEEDBACK_STATE_KEY);
        if (stored) {
            console.log('保存された状態:', JSON.parse(stored));
        } else {
            console.log('保存された状態: なし');
        }
...
// 状態強制クリア関数（デバッグ用）
window.clearFeedbackState = function() {
    if (!window.yourNewsApp) {
        console.error('YourNewsApp not initialized');
        return;
    }
    
    window.yourNewsApp.processingArticles.clear();
    window.yourNewsApp.buttonCooldowns.clear();
    localStorage.removeItem(window.yourNewsApp.FEEDBACK_STATE_KEY);
    ...
    console.log('✅ フィードバック状態を強制クリアしました');
};
