...
async function loadOPML() {
  ...
  try {
    ...
    outlines.forEach(outline => {
      if (outline.outline) {
        // フォルダの場合
        const folderId = `folder_${outline.$.text.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase()}`;
        folders.push({
          id: folderId,
          name: outline.$.text,
          order: folderOrder++
    ...
    // デフォルトフォルダを追加
    if (!folders.find(f => f.id === 'folder_uncategorized')) {
      folders.push({
...
async function fetchAndParseRSS(url, title, folderName = '未分類') {
  try {
    console.log(`🔍 [${title}] RSS取得開始: ${url} (${folderName})`);
...
async function parseRSSItem(item, sourceUrl, feedTitle, folderName = '未分類') {
  try {
    ...
    return {
      ...
      rssSource: feedTitle,
      folderName: folderName,  // 【NEW】フォルダ名を追加
...
async function main() {
  try {
    ...
    const opmlResult = await loadOPML();
    const feeds = opmlResult.feeds;
    const folders = opmlResult.folders;
    ...
    console.log(`📊 フィード情報: ${feeds.length}個のRSSフィード、${folders.length}個のフォルダを処理します`);
    ...
    folders.forEach(folder => {
      const folderArticles = limitedArticles.filter(article => article.folderName === folder.name);
      console.log(`     ${folder.name}: ${folderArticles.length}件`);
    });
