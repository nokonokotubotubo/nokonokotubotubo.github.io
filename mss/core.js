// Minews PWA - Core Module (データ処理・管理機能)
(function() {
'use strict';
...
const FolderManager = {
    ...
    validateFolder: folder => 
        folder && typeof folder.name === 'string' && 
        folder.name.trim().length > 0 && folder.name.trim().length <= 50,

    getColorName: colorValue => 
        CONFIG.FOLDER_COLORS.find(c => c.value === colorValue)?.name || 'カスタム',
    ...
    extractDomainFromSource: source => 
        source.includes('.') ? source.toLowerCase().replace(/^www\./, '') : source.toLowerCase(),
    ...
    useRSSManager() {
        ...
        return {
            ...
            addRSSFeed(url, title, folderId = 'uncategorized') {
                const newFeed = {
                    id: `rss_${Date.now()}`,
                    url, title: title || 'Unknown Feed',
                    folderId, lastUpdated: new Date().toISOString(),
                    isActive: true
                };
            ...
            async fetchAllFeeds() {
                ...
                for (const feed of DataHooksCache.rssFeeds.filter(f => f.isActive)) {
                    try {
                        ...
                        feedResults.push({
                            name: feed.title, success: true,
                            added: addedCount, total: parsed.articles.length
                        });
                    } catch (error) {
                        totalErrors++;
                        feedResults.push({
                            name: feed.title, success: false,
                            error: error.message
                        });
                    }
                }
...
if (typeof window !== 'undefined') {
    ...
    window.DataHooksCache = DataHooksCache;
    window.FolderManager = FolderManager;
    window.RSSProcessor = RSSProcessor;
    window.AIScoring = AIScoring;
