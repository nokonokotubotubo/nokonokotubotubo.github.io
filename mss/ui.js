// Minews PWA - UI・表示レイヤー（チェックボックス完全版）
(function() {
    'use strict';
        ...
        try {
            ...
            if (stored) {
                ...
                return {
                    ...
                    showRead: parsed.showRead !== false,
                    showReadLater: parsed.showReadLater !== false
                };
            }
        } catch (error) {
    ...
    const saveFilterState = (state) => {
        try {
            localStorage.setItem('minews_filterState', JSON.stringify(state));
            console.log('フィルター状態を保存:', state);
        } catch (error) {
    ...
    window.state = {
        ...
        showUnread: initialFilterState.showUnread,
        showRead: initialFilterState.showRead,
        showReadLater: initialFilterState.showReadLater,
        showModal: null,
        articles: [],
        isLoading: false,
        lastUpdate: null,
    ...
    window.setState = (newState) => {
        console.log('setState呼び出し:', newState);
        const oldState = { ...window.state };
        window.state = { ...window.state, ...newState };
        
        // フィルター関連の状態変更時は自動保存
        if (newState.selectedSource !== undefined || 
            newState.showUnread !== undefined ||
            newState.showRead !== undefined ||
            newState.showReadLater !== undefined) {
            
            const filterState = {
                selectedSource: window.state.selectedSource,
                showUnread: window.state.showUnread,
                showRead: window.state.showRead,
                showReadLater: window.state.showReadLater
            };
            
            saveFilterState(filterState);
        }
        
        console.log('状態更新完了:', { old: oldState, new: window.state });
        window.render();
    };
    ...
    const initializeData = () => {
        ...
        console.log('データ初期化完了 - 記事数:', articlesData.length);
        console.log('初期フィルター状態:', {
            selectedSource: window.state.selectedSource,
            showUnread: window.state.showUnread,
            showRead: window.state.showRead,
            showReadLater: window.state.showReadLater
    ...
    const handleCheckboxChange = (type, checked) => {
        console.log(`チェックボックス変更: ${type} = ${checked}`);
        const newState = {};
            ...
            <nav class="nav">
                ...
                <div class="nav-filters-mobile">
                    ...
                    <div class="checkbox-group">
                        <label class="checkbox-label">
                            <input type="checkbox" 
                                   id="showUnread-mobile"
                                   name="showUnread"
                                   ${window.state.showUnread ? 'checked' : ''}
                                   onchange="handleCheckboxChange('showUnread', this.checked)">
                            <span class="checkbox-text">未読</span>
                        </label>
                        ...
                        <label class="checkbox-label">
                            <input type="checkbox" 
                                   id="showRead-mobile"
                                   name="showRead"
                                   ${window.state.showRead ? 'checked' : ''}
                                   onchange="handleCheckboxChange('showRead', this.checked)">
                            <span class="checkbox-text">既読</span>
                        </label>
                        
                        <label class="checkbox-label">
                            <input type="checkbox" 
                                   id="showReadLater-mobile"
                                   name="showReadLater"
                                   ${window.state.showReadLater ? 'checked' : ''}
                                   onchange="handleCheckboxChange('showReadLater', this.checked)">
                            <span class="checkbox-text">後で読む</span>
                        </label>
                    </div>
                </div>
                ...
                <div class="nav-filters desktop-only">
                    ...
                    <div class="checkbox-group">
                        <label class="checkbox-label">
                            <input type="checkbox" 
                                   id="showUnread-desktop"
                                   name="showUnreadDesktop"
                                   ${window.state.showUnread ? 'checked' : ''}
                                   onchange="handleCheckboxChange('showUnread', this.checked)">
                            <span class="checkbox-text">未読</span>
                        </label>
                        ...
                        <label class="checkbox-label">
                            <input type="checkbox" 
                                   id="showReadLater-desktop"
                                   name="showReadLaterDesktop"
                                   ${window.state.showReadLater ? 'checked' : ''}
                                   onchange="handleCheckboxChange('showReadLater', this.checked)">
                            <span class="checkbox-text">後で読む</span>
                        </label>
    ...
    const getFilteredArticles = () => {
        ...
        // チェックボックスフィルター
        const beforeCheckboxFilter = filtered.length;
        filtered = filtered.filter(article => {
            // 後で読むが有効で、記事が後で読む状態の場合
            if (article.readLater && window.state.showReadLater) {
                return true;
            }
            
            // 後で読むでない記事の場合、既読/未読状態をチェック
            if (!article.readLater) {
                if (article.readStatus === 'read' && window.state.showRead) {
                    return true;
                }
                if (article.readStatus === 'unread' && window.state.showUnread) {
                    return true;
                }
            }
        ...
        console.log(`チェックボックスフィルター: ${beforeCheckboxFilter} → ${filtered.length}`);
    ...
    const renderArticleList = () => {
        ...
        if (articles.length === 0) {
            return '<div class="empty-message">表示する記事がありません<br><small>フィルターを確認してください</small></div>';
        }
    ...
    window.render = () => {
        ...
        console.log('レンダリング開始 - チェックボックス版');
        console.log('フィルター状態:', {
            selectedSource: window.state.selectedSource,
            showUnread: window.state.showUnread,
            showRead: window.state.showRead,
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            console.log('チェックボックス版 - DOM読み込み完了、初期化開始');
            initializeData();
            initializeGistSync();
            window.render();
        });
    } else {
        console.log('チェックボックス版 - DOM既に読み込み済み、初期化開始');
        initializeData();
        initializeGistSync();
        window.render();
    }
